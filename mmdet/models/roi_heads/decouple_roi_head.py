...
class MultipleScaleFeature(nn.Module):
    ...
    def builder_inter_conv(self):
        ...
        for index in range(self.inter_num):
            tmp_conv.append(ConvModule(in_channels=self.in_channels, out_channels=self.in_channels, kernel_size=3, stride=1, padding=1))
        return nn.ModuleList(tmp_conv)

    @auto_fp16()
    def forward(self, x):
        tmp = []
        for block in self.mix_block:
            x = block(x)
            tmp.append(x)
        return tmp

class DynamicSpatialSelect(nn.Module):
    def __init__(self, head_num=3):
    ...
    @auto_fp16()
    def forward(self, x):
        cat_fea = torch.cat(x, dim=1)
        max_x = torch.max(cat_fea, dim=1, keepdim=True)[0]
        mean_x = torch.mean(cat_fea, dim=1, keepdim=True)
        cat_fea = torch.cat([max_x, mean_x], dim=1)
        att = self.act(self.conv1(cat_fea))
        x1 = att[:, 0, ...].unsqueeze(1) * x[0]
        x2 = att[:, 1, ...].unsqueeze(1) * x[1]
        x3 = att[:, 2, ...].unsqueeze(1) * x[2]
        return x1 + x2 + x3

@HEADS.register_module()
class DecoupleRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    """Simplest base roi head including one bbox head and one mask head."""
    # def __init__(self, task_num=2):
    #     super(DecoupleRoIHead, self).__init__()
    #     #shared decouple layer
    #     self.mix_scale = MultipleScaleFeature()
    #     self.reg_select = DynamicSpatialSelect()
    #     self.cls_select = DynamicSpatialSelect()
    ...
    def init_bbox_head(self, bbox_roi_extractor, bbox_head):
        ...
        self.mix_scale = nn.ModuleList([MultipleScaleFeature() for _ in range(5)])
        self.center_select = nn.ModuleList([DynamicSpatialSelect() for _ in range(5)])
        self.wh_select = nn.ModuleList([DynamicSpatialSelect() for _ in range(5)])
        self.cls_select = nn.ModuleList([DynamicSpatialSelect() for _ in range(5)])
    ...
    def _bbox_forward(self, x, rois, img_metas=None):
        ...
        # tmp = multi_apply(self.mix_scale, list(x))
        # reg_feats = multi_apply(self.reg_select, tmp)
        # cls_feats = multi_apply(self.cls_select, tmp)
        center_feats = []
        wh_feats = []
        cls_feats = []
        for index, fea in enumerate(list(x)):
            refine_fea = self.mix_scale[index](fea)
            center_feats.append(self.center_select[index](refine_fea))
            wh_feats.append(self.wh_select[index](refine_fea))
            cls_feats.append(self.cls_select[index](refine_fea))
        center_bbox_feats = self.bbox_roi_extractor(center_feats[:self.bbox_roi_extractor.num_inputs], rois)
        cls_bbox_feats = self.bbox_roi_extractor(cls_feats[:self.bbox_roi_extractor.num_inputs], rois)
        wh_bbox_feats = self.bbox_roi_extractor(wh_feats[:self.bbox_roi_extractor.num_inputs], rois)
        ...
        if self.with_shared_head:
            ...
            center_bbox_feats = self.shared_head(center_bbox_feats)
            cls_bbox_feats = self.shared_head(cls_bbox_feats)
            wh_bbox_feats = self.shared_head(wh_bbox_feats)
        cls_score, bbox_pred = self.bbox_head(center_bbox_feats, wh_bbox_feats, cls_bbox_feats)
        #print('cls_score:{}'.format(cls_score.size())) #1024*9
        #print('bbox_pred:{}'.format(bbox_pred.size())) #1024*32

        bbox_results = dict(
            cls_score=cls_score, bbox_pred=bbox_pred, bbox_feats=(center_bbox_feats, cls_bbox_feats))
        return bbox_results
