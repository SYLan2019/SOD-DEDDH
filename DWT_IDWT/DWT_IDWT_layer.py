...
class DWT_2D(Module):
    ...
    def get_matrix(self, data_device):
        """
        生成变换矩阵
        generating the matrices: \mathcal{L}, \mathcal{H}
        :return: self.matrix_low = \mathcal{L}, self.matrix_high = \mathcal{H}
        ...
        if torch.cuda.is_available():
            self.matrix_low_0 = torch.Tensor(matrix_h_0).to(data_device)
            self.matrix_low_1 = torch.Tensor(matrix_h_1).to(data_device)
            self.matrix_high_0 = torch.Tensor(matrix_g_0).to(data_device)
            self.matrix_high_1 = torch.Tensor(matrix_g_1).to(data_device)
        else:
            self.matrix_low_0 = torch.Tensor(matrix_h_0)
            self.matrix_low_1 = torch.Tensor(matrix_h_1)
            self.matrix_high_0 = torch.Tensor(matrix_g_0)
